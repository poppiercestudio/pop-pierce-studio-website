...
function cancelAddGalleryItem() {
    const galleryManager = document.getElementById('galleryManager');
    const newItemDiv = galleryManager.querySelector('.gallery-item-admin');
    if (newItemDiv && newItemDiv.style.border.includes('667eea')) {
        newItemDiv.remove();
    }
}
...
async function deleteGalleryItem(index) {
    if (confirm('¿Estás seguro de que quieres eliminar este elemento de la galería?')) {
        ...
        if (isGitHubConfigured()) {
            try {
                await saveToGitHub();
                showNotification('Elemento eliminado de GitHub', 'success');
            } catch (error) {
