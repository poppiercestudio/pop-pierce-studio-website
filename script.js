    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.gallery && data.gallery.length > 0) {
                // Combinar con imágenes originales si es necesario
                const currentLocal = JSON.parse(localStorage.getItem(GALLERY_KEY) || '[]');
                const githubGallery = data.gallery;
                
                // Si la galería de GitHub tiene menos elementos que la local, 
                // mantener la local (tiene las originales migradas)
                if (currentLocal.length > githubGallery.length) {
                    // La local tiene más, probablemente incluye las originales
                    // No sobrescribir
                } else {
                    // Usar la de GitHub
                    localStorage.setItem(GALLERY_KEY, JSON.stringify(githubGallery));
                }
            }
        }
    } catch (error) {
...
// También escuchar cambios del mismo tab usando polling (cada 2 segundos)
