    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.gallery && Array.isArray(data.gallery)) {
                // SIEMPRE usar la versión de GitHub si está disponible
                // Esto asegura que los cambios (eliminaciones, etc.) se reflejen
...
setInterval(function() {
    ...
    if (now - lastGitHubCheck > GITHUB_CHECK_INTERVAL) {
        ...
        fetch('data.json?' + Date.now())
            ...
            .then(data => {
                if (data.gallery && Array.isArray(data.gallery)) {
                    const githubGalleryStr = JSON.stringify(data.gallery);
                    ...
                    if (githubGalleryStr !== currentGalleryStr) {
                        ...
                        localStorage.setItem('pop_pierce_gallery', JSON.stringify(data.gallery));
                        loadGalleryFromStorage();
                    }
                }
