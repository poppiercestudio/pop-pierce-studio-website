...
async function loadLogosFromStorage() {
    ...
    try {
        // Intentar desde URL pública de GitHub Pages primero
        let dataUrl = 'data.json?' + Date.now();
        
        // Si estamos en local (file://), intentar desde GitHub Pages
        if (window.location.protocol === 'file:') {
            // Usar la configuración de GitHub si está disponible
            if (typeof GITHUB_CONFIG !== 'undefined' && GITHUB_CONFIG && GITHUB_CONFIG.owner && GITHUB_CONFIG.repo) {
                const { owner, repo } = GITHUB_CONFIG;
                dataUrl = `https://${owner}.github.io/${repo}/data.json?t=${Date.now()}`;
            } else {
                // Fallback a la configuración por defecto
                dataUrl = 'https://poppiercestudio.github.io/pop-pierce-studio-website/data.json?t=' + Date.now();
            }
        }
